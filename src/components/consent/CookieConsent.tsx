...
export default function CookieConsent() {
  ...
  // On mount: load any saved choice and inject the consented scripts.
  useEffect(() => {
  ...
  // First-time visitors don't get the banner until they've actually entered
  // the site via the splash -- otherwise it (and the TermsGate popup that
  // follows) would pile up on top of / behind the splash overlay. The
  // `entered` signal comes from the `splash-entered` class on <html>.
  useEffect(() => {
    if (entered && !decided) setShowBanner(true);
  }, [entered, decided]);
        ...
        >
          <div className="relative max-h-[88vh] w-full max-w-[540px] overflow-y-auto border border-text-body/20 bg-background-alt p-6 shadow-2xl sm:p-8">
            ...
            <button
              ...
              className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full text-text-accent/70 transition-colors hover:bg-black/10 hover:text-text-accent"
            >
                  ...
                  {cat.locked ? (
                    ...
                    <button
                      ...
                      className={`mt-1 flex h-6 w-11 shrink-0 items-center rounded-full px-0.5 transition-colors ${
                        draft[cat.key] ? "bg-[#211F21] justify-end" : "bg-[#211F21]/25 justify-start"
                      }`}
                    >
                      <span
                        className={`h-5 w-5 rounded-full ${draft[cat.key] ? "bg-[#E8E3CF]" : "bg-[#211F21]"}`}
                      />
                    </button>
                  )}
                </li>
            ...
            <div className="mt-6 min-h-[20px]">
              {dirty && (
                <p className="flex items-center gap-2 font-body text-xs text-text-accent">
                  <span className="inline-block h-2 w-2 shrink-0 rounded-full bg-[#211F21]" />
